...
Exciting! We see sales declining slightly in the past few months in this subset, and the model accounts for that and estimates 50 items sold for the next month in the cycle.. 
 
 I personal see both the (1, 1, 2) and the (0, 1, 1) models working pretty accurately from these quick tests.. I need to write up a function to run through the store item combos and fit the model and report it to a list.. I'll finally take the list and reorder it to the sample submission and test the fits on kaggle.
...
~40% of contestants.  I'm happy with it for my first time running a time series model. This model may also be running into trouble due to over-generalizing the auto-regressive and moving average values for each shop-item combo.. It's definately not the strongest for each individual combination. 

Regardless, I'm happy with it.  A root mean squared error of only 1.16 is pretty decent for predicting how many items a store will sell based on only 34 months of data.  Including some more details about the items (like category or price) could enhance this even more I'm sure.  
As my goal was to fit and test a time-series model though, I'm happy with what I achieved here.
